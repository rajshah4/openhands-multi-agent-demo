...
The original demos run the same three-phase pipeline:
...
This repo demonstrates **three runtime patterns** for running multiple agents.
...
📖 **[Read the workflow patterns guide →](WORKFLOW_PATTERNS.md)** for parent-child
delegation, polling continuation loops, and when to choose each workflow shape.
...
| Workflow pattern | Example | Use when |
| --- | --- | --- |
| **Linear pipeline** | `cloud_conversations.py`, `shared_workspace.py`, `multi_server_isolation.py` | The work is bounded and should run implement -> test -> review. |
...
| `parent_child_supervisor.py` | Workflow pattern — parent delegates children and gates on final responses |
| `polling_continuation_loop.py` | Workflow pattern — cron-friendly state check, spawn, log, and exit loop |
| `WORKFLOW_PATTERNS.md` | Workflow-pattern catalog and decision guide |
