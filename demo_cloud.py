#!/usr/bin/env python3
"""
Cloud-Native Multi-Harness Demo (ACP)
======================================
...
What happens:
  ...
  3. Runs demo.py which uses ACPAgent (real ACP protocol) for Claude Code
...
def build_task_prompt(args):
    ...
    if args.no_claude:
        demo_cmd = f"python demo.py --no-claude {task_flag}"
        harness_desc = "OpenHands agents only (no ACP)"
    else:
        demo_cmd = f"python demo.py {task_flag}"
        harness_desc = "Claude Code (ACP) + OpenHands"

    prompt = f"""Run the multi-agent orchestration demo from this repo.

This demo uses multiple agent harnesses: {harness_desc}.
...
5. After the demo completes, show me:
   - What files were created
   - The review findings (if any)
   - The total cost

IMPORTANT: The demo.py script uses ACPAgent which spawns Claude Code
via the Agent Client Protocol. This is a real ACP integration, not just
a CLI wrapper. Let the script run to completion — it manages its own
